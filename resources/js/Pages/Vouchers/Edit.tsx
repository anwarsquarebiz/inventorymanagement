...
export default function Edit({ voucher, users, shapes, products }: VoucherEditProps) {
    ...
    const initialItems: LineItem[] = voucher.items.map((it) => ({
        id: String(it.id),
    ...
    const calculateTotals = () => {
        const totalPcs = (data.items ?? []).map((item: any) => Number(item.pcs) || 0).reduce((sum: number, item: number) => sum + item, 0);
        const totalWeight = (data.items ?? []).map((item: any) => parseFloat(String(item.weight)) || 0).reduce((sum: number, item: number) => sum + item, 0);
            ...
            <div className="space-y-6">
                <div className="flex items-center space-x-4">
                    ...
                    <div>
                        ...
                        <p className="text-gray-600">Update voucher information and line items</p>
                    </div>
                </div>
                    ...
                    <Card className="p-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div className="p-4 bg-gray-50 rounded-lg">
                                ...
                                <p className="text-xl font-semibold text-gray-900">{Number(totalWeight).toFixed(2)} ct</p>
                            </div>
                        </div>
                    </Card>
